...
iconFile = path.join(thisFolder,'eyes.png')
tooltip = 'Gaze Tracker Mouse: query mouse position from Gaze'

class GazeComponent(BaseComponent):
    """An event class for checking the mouse location at given timepoints"""
    categories = ['Responses']
    def __init__(self, exp, parentName, name='gaze tracker',
    ...
    def writeInitCode(self,buff):
        ...
        # Start gaze tracker
        t = threading.Thread(target=self.run_c_sharp_code)
        t.start()
    ...
    def writeRoutineStartCode(self,buff):
        ...
        self.binaryNamedPipe2('settings')
    ...
    def run_c_sharp_code(self):
        dir = 'C:\\Users\\Devin\\Desktop\\gazetrackinglib\\gazetrackinglib\\trunk\\Debug'
        dir = 'J:\\psych\\gazetrackinglib\\gazetrackinglib\\trunk\\Debug'
        time.sleep(1)
        print('Starting C Sharp Window')
        os.chdir(dir)
        os.system(dir + '\\GazeTrackerUI.exe')
    
    def binaryNamedPipe2(self, message):
        f = open(r'\\.\pipe\NPtest', 'r+b', 0)
        f.write(struct.pack('I', len(message)) + message)   # Write str length and str
        print('Wrote: {}'.format(message))
        f.flush()
        f.close()
